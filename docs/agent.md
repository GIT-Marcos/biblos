...
| F | No            | Si                      | —             | No                    | DELETE              |
| G | No            | Si                      | —             | Si                    | SKIP                |
| H | Si            | Si                      | No            | Si                    | REACTIVATE + UPDATE |

**Notas sobre la clasificación:**

- Los archivos con hash fallido (contentHash = null) se excluyen del pipeline.
- Las operaciones se ordenan: RENAME → UPDATE → REACTIVATE → CREATE → DELETE. Nota: El caso H (archivo soft-deleted
  reaparece con hash diferente) se ejecuta como REACTIVATE + UPDATE.
...
| H7 | Path > 260 caracteres en Windows (MAX_PATH)                                                                                        | Usar prefijo `\\?\` en `Path` para operaciones de I/O. Requisitos: (1) Habilitar "Enable Win32 long paths" en Group Policy o Registry (opcional pero recomendado), (2) En Java, usar `Path.of("\\\\?\\" + path)` o `Paths.get("\\\\?\\")` para paths largos, (3) No requiere manifest (Java no usa .exe). Si no se habilita → WARN: "Path demasiado largo, archivo excluido: [path]"                                                                                  | Sin habilitación: archivos excluidos silenciosamente. Con habilitación: funciona pero puede causar problemas con herramientas que no soportan paths largos                |
...
| R6    | Mismo hash en múltiples sources                                     | selectBestMatch: 1) Filtrar sources con mismo content_hash, 2) Si hay múltiples candidatos: a) Priorizar: active (deletedAt=NULL) > orphan (deletedAt!=NULL), b) Dentro de cada categoría: i) Preferir el que coincida en path_lower con el path esperado (misma carpeta de autor), ii) Si ninguno coincide → elegir el de menor path_lower (orden alfabético), iii) Si el candidato ya fue "reclamado" por otro RENAME en el mismo scan → skip y elegir el siguiente, 3) Si no hay candidatos → CREATE (source nuevo)                                                                    | Prioriza activos; puede elegir incorrectamente si hay duplicados legítimos                                                                                 |
...
| G4 | Archivos sin extensión en FS (ej: `documento`, `sin_extension`)                                                                                                          | WARN: "Archivo sin extensión soportada, excluido: [path]" → skip silencioso                                      | Excluye archivos que el usuario debe renombrar con extensión correcta         |
| G5 | Archivos con extensión múltiple (ej: `libro.pdf.bak`, `copia.epub.tmp`)                                                                                                  | Procesar solo la extensión final (ej: `.bak`, `.tmp`) → WARN: "Extensión no soportada: [extensión final]" → skip | Ignora archivos de backup/temporarios; mantiene el catálogo limpio            |
| G6 | Archivos con extensión similar no soportada (ej: `.mht` en lugar de `.mhtml`)                                                                                            | WARN: "Extensión no soportada: [extensión] (¿quiso decir [extensión similar]?)' → skip                           | Ayuda al usuario a identificar extensiones casi-soportadas                    |
| G7 | Archivos con caracteres Unicode especiales (emojis, caracteres diacríticos)                                                                                              | Preservar original (el FS ya los maneja); normalizar a NFC antes de almacenar                                    | Más fiel al FS; evita problemas de codificación                               |
| G8 | Archivos con nombres muy largos (>255 caracteres en Windows)                                                                                                             | WARN: "Nombre de archivo excede 255 caracteres, excluido: [path]" → skip silencioso                              | Previene errores del FS; el usuario debe acortar el nombre                    |
